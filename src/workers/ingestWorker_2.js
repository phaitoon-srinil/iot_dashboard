...
const worker = new Worker(
  ...
  async (job) => {
    ...
    if (deviceKind === 'weatherStation') {
      ...
      // ================= RAIN =================
      const currentRainDay = s.rain_day ?? 0;
      ...
      if (prev.length && prev[0].rain_day !== null) {
        const prevRain = prev[0].rain_day;

        if (currentRainDay < prevRain) {
          hourlyRain = currentRainDay;
        } else {
          hourlyRain = currentRainDay - prevRain;
        }
      }
